(function() {
    ...
    function aggregatedService($http, $q, aggregatedModelMockService) {
        ...
        function toggle_status(item) {
            if(item.read){
                return aggregatedModelMockService.set_unread(item.id);
            } else {
                return aggregatedModelMockService.set_read(item.id);
            }
        }
    ...
    aggregatedService.$inject = ["$http", "$q", "aggregatedModelMockService"];
